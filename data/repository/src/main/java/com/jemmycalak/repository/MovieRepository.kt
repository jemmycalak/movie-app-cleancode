...
interface MovieRepositoryInterface{
    suspend fun getListMovie(filter:Int, page:Int,shouldFetch:Boolean):LiveData<Resource<MovieModel>>
...
class MovieRepository(private val source:MovieDataSource,
                      ...
                      private val gson:Gson):MovieRepositoryInterface{
    override suspend fun getListMovie(filter:Int, page:Int,shouldFetch:Boolean): LiveData<Resource<MovieModel>> {
        return object : NetworkBoundResource<MovieModel, MovieModel>(gson){
            ...
            override suspend fun saveCallResults(data: MovieModel) {
                ...
                dao.saveMovieModel(MovieTable(data.page, data.totalPages, data.totalResults, dateNow.toString()))
            ...
            override suspend fun loadFromDb(): MovieModel? {
                ...
                return if(model != null ) MovieModel(model.page, result, model.totalPages, model.totalResults) else null
            }

            override suspend fun createCallAsync(): Response<MovieModel> = source.getListMovie(filter, page)
    ...
    override suspend fun getReviewMovie(idMovie: String): LiveData<Resource<ReviewMovie>> {
        return object : NetworkBoundResource<ReviewMovie, ReviewMovie>(gson){
            ...
            override suspend fun createCallAsync(): Response<ReviewMovie> = source.getListReviewMovie(idMovie)
        }.build().asLiveData()
    }
